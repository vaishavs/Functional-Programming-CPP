...
The view adapters defined under ```std::views```, such as ```std::views::filter``` and ```std::views::transform``` don’t immediately process data. Instead, they create a view — a lightweight object that does not own or copy data but just defines how elements should be seen. This allows for lazy evaluation, where the operations are defined immediately but logic is only executed when we actually iterate over the final result.
