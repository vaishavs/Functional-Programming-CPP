...
int main() {
    ...
    //          5  --(*2)-->  Box<int>{10}
    std::cout << numberBox.map([](int num){ return num * 2; }).value << '\n';        // 10
    ...
    std::cout << Box{+[](int num){ return num + 100; }}.ap(numberBox).value << '\n';  // 105

    // MONAD: the lambda returns a *box* (Box<int>{num*num}); bind flattens it,
    //        so the result is Box<int>{25}, not Box<Box<int>>.
    std::cout << numberBox.bind([](int num){ return Box{num * num}; }).value << '\n'; // 25
}
